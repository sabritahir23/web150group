...
if(!function_exists('makeLinks'))
{
    function makeLinks($nav)
    {
        $myReturn  = '';
        /*
            //here's how we usually get this value
            $active = $ci->config->get_item('active');//sets CSS class as active for Users
            
            Note below we had to get external reference to $ci and had to investigate via 
            var_dump() to determine how to retrieve the value externall - note the config->config reference
        */
        $ci =& get_instance();//can't use $this in external context
        $active = $ci->config->config['active'];
